...
function Page({ children }: PageProps) {
  return (
    <Stack direction="column" sx={{ width: "100%", alignItems: "center" }}>
      <Stack
        direction="column"
        sx={{ width: "750px", alignItems: "left", paddingTop: 8, gap: 2 }}
      >
        {children}
      </Stack>
    </Stack>
...
function Genealogy({ tree }: GenealogyProps) {
  ...
  const expand = (i: number, parent: Academic) =>
    setAcademics([...academics.slice(0, i + 1), parent]);
...
function Individual(props: IndividualProps) {
  return props.academic.bio ? (
    <IndividualLong {...props} />
  ) : (
    <IndividualBrief {...props} />
  );
}
  ...
  return (
    ...
    >
      <Stack direction="row" sx={{ alignItems: "end", gap: 1 }}>
        <Typography variant="h1" sx={{ marginBottom: -0.9 }}>
          {props.academic.name}
        </Typography>
...
function IndividualBrief(props: IndividualProps) {
  return (
    <Stack direction="row" sx={{ alignItems: "center", gap: 1 }}>
      ...
      <Stack
        direction="column"
        sx={{ width: "100%", alignItems: "left", gap: 0.5 }}
      >
        <Stack
          direction="row"
          sx={{ width: "100%", alignItems: "end", gap: 1 }}
        >
          <Typography variant="h2" sx={{ marginBottom: -0.2 }}>
            {props.academic.name}
          </Typography>
          <GenealogyButtons {...props} />
        </Stack>
        <Stack direction="row" sx={{ alignItems: "center", gap: 0.5 }}>
          {props.academic.school && (
            <Typography variant="body2">{props.academic.school}</Typography>
          )}
          {props.academic.grad && (
            <Typography variant="body2">{props.academic.grad}</Typography>
          )}
        </Stack>
      </Stack>
    </Stack>
...
function GenealogyButtons(props: IndividualProps) {
  const inactiveParents = props.academic.parents.filter(
    (p) => p.name !== props.activeParent?.name,
  );
  return inactiveParents.length === 0 ? null : (
    <>
      <Typography variant="body2">
        {inactiveParents.length === props.academic.parents.length
          ? "advised by"
          : "also advised by"}
      </Typography>
      {inactiveParents.map((parent, i) => (
        <GenealogyButton
          key={`${props.academic.name}${i}`}
          {...props}
          parent={parent}
        />
      ))}
...
function GenealogyButton({
  activeParent,
  parent,
  collapse,
  expand,
}: GenealogyButtonProps) {
  return (
    <Link
      variant="body2"
      underline="hover"
      sx={{ cursor: "pointer" }}
      onClick={() =>
        activeParent?.name === parent.name ? collapse() : expand(parent)
      }
    >
      {parent.name}
    </Link>
...
function Cite({ citation }: CiteProps) {
  const authors = citation.authors
    .map((author, i) =>
      i > 0 && i === citation.authors.length - 1 ? "and " + author : author,
    )
    .join(", ");
  return (
    <Stack direction="row" sx={{ width: "100%", alignItems: "begin", gap: 1 }}>
      ...
      >
        ...
        <Stack
          direction="row"
          sx={{ width: "100%", alignItems: "center", gap: 0.5 }}
        >
          {citation.bib && (
            <DocLink label="bib" prefix="bib" path={citation.bib} />
          )}
          {citation.text && (
            <DocLink label="pdf" prefix="text" path={citation.text} />
          )}
          {citation.deck && (
            <DocLink label="deck" prefix="deck" path={citation.deck} />
          )}
          {citation.errata && (
            <DocLink label="errata" prefix="errata" path={citation.errata} />
          )}
        </Stack>
      </Stack>
    </Stack>
